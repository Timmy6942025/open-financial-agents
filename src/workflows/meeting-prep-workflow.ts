 ...
 *   - news-reader: read+grep only, NO MCP, reads UNTRUSTED client emails
...
export const meetingPrepWorkflow = createWorkflow({
  ...
  .then(
    createStep({
      id: "read-news",
      description: "Read UNTRUSTED client emails and news articles, extract relevant items (read-only, no MCP)",
      inputSchema: z.object({
        profile: z.string(),
      }),
      outputSchema: z.object({
  ...
  .then(
    createStep({
      ...
      execute: async ({ inputData, mastra }) => {
        ...
        return { briefingPath: result || "./out/briefing-pack.pptx" };
      },
    })
  )
  .commit();
