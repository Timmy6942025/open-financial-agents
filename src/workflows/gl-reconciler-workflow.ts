...
export const glReconcilerWorkflow = createWorkflow({
    ...
    createStep({
      ...
      execute: async ({ inputData, mastra }) => {
        try {
          ...
          throw new Error(`gl-reconciler-reader failed: ${err.message}`);
        }
      },
    })
  )
  .then(
    createStep({
      ...
      execute: async ({ inputData, mastra }) => {
        try {
          if (inputData.handoff) return { verified: inputData.breaks, handoff: inputData.handoff };
          ...
          throw new Error(`gl-reconciler-critic failed: ${err.message}`);
        }
      },
    })
  )
  .then(
    createStep({
      ...
      execute: async ({ inputData, mastra }) => {
        try {
          if (inputData.handoff) return { report: inputData.verified, handoff: inputData.handoff };
          ...
          throw new Error(`gl-reconciler-resolver failed: ${err.message}`);
        }
      },
    })
  )
