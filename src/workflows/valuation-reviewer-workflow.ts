    ...
    createStep({
      ...
      execute: async ({ inputData, mastra }) => {
        try {
          ...
          throw new Error(`valuation-package-reader failed: ${err.message}`);
        }
      },
    })
  )
      ...
      execute: async ({ inputData, mastra }) => {
        try {
          if (inputData.handoff) {
            return { reviewSummary: inputData.marks, handoff: inputData.handoff };
          }
          ...
          throw new Error(`valuation-runner failed: ${err.message}`);
        }
      },
    })
  )
  .then(
    createStep({
      ...
      execute: async ({ inputData, mastra }) => {
        try {
          if (inputData.handoff) {
            return { lpPackPath: inputData.reviewSummary, handoff: inputData.handoff };
          }
          ...
          throw new Error(`valuation-publisher failed: ${err.message}`);
        }
      },
    })
  )
